...
export async function fetchAutoScalersRegionPolicy(
  ...
  await jobState.iterateEntities(
    ...
    async (computeEngineAutoscalerEntity) => {
      const regionAutoscaler = getRawData<compute_v1.Schema$Autoscaler>(
      ...
      await jobState.addEntity(
        createAutoScalersRegionPolicyEntity(
          regionAutoscaler?.autoscalingPolicy,
          client.projectId,
          regionAutoscaler.id,
          regionAutoscaler.name,
        ),
      );
    },
  );
